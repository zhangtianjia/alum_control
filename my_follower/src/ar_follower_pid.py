...
class ARFollower_pid():
    def __init__(self):
        rospy.init_node("ar_follower_pid")
	...
	self.pid_control_position=PID(2,3,0.01,0.1)
        self.pid_control_angular=PID(2,3,0.01,0.1)
    ...
    def set_cmd_vel(self, msg):
        ...
        if abs(target_offset_y) > self.y_threshold:
            ...
            speed=self.pid_control_position.update_control(target_offset_y)
        ...
        if abs(target_offset_x - self.goal_x) > self.x_threshold:
            speed = self.pid_control_angular.update_control(target_offset_x - self.goal_x)
            if speed < 0:
                speed *= 1.5
